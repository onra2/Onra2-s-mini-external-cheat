...
int DrawString(char* String, int x, int y, int r, int g, int b, ID3DXFont* ifont)
{
	...
	FontPos.left = x;
	FontPos.top = y;
	ifont->DrawTextA(0, (LPCSTR)String, strlen((LPCSTR)String), &ShadowPos, DT_NOCLIP, D3DCOLOR_ARGB(255, r / 3, g / 3, b / 3));
	ifont->DrawTextA(0, (LPCSTR)String, strlen((LPCSTR)String), &FontPos, DT_NOCLIP, D3DCOLOR_ARGB(255, r, g, b));
	return 0;
}
